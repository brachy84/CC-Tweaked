...
public class TurtleBrain implements ITurtleAccess {
    ...
    private final Map<TurtleSide, NBTTagCompound> m_upgradeNBTData = new EnumMap<>(TurtleSide.class);
    ...
    public void readFromNBT(NBTTagCompound nbt) {
        ...
        if (nbt.hasKey("owner", Constants.NBT.TAG_COMPOUND)) {
            ...
            m_owningPlayer = new GameProfile(new UUID(owner.getLong("upper_id"), owner.getLong("lower_id")), owner.getString("name"));
        } else {
            m_owningPlayer = null;
        }
        ...
        if (nbt.hasKey("subType")) {
            ...
            if (nbt.hasKey("leftUpgrade")) {
                leftUpgrade = nbt.getTagId("leftUpgrade") == Constants.NBT.TAG_STRING ? TurtleUpgrades.get(nbt.getString("leftUpgrade")) :
                              TurtleUpgrades.get(nbt.getShort("leftUpgrade"));
            }
            if (nbt.hasKey("rightUpgrade")) {
                rightUpgrade =
                    nbt.getTagId("rightUpgrade") == Constants.NBT.TAG_STRING ? TurtleUpgrades.get(nbt.getString("rightUpgrade")) :
    ...
    public void readDescription(NBTTagCompound nbt) {
        ...
        if (anim != m_animation && anim != TurtleAnimation.Wait && anim != TurtleAnimation.ShortWait && anim != TurtleAnimation.None) {
    ...
    @Nonnull
    @Override
    public Object[] executeCommand(@Nonnull ILuaContext context, @Nonnull ITurtleCommand command)
    ...
    public Vec3d getRenderOffset(float f) {
        switch (m_animation) {
            ...
            case MoveDown: {
                ...
                return new Vec3d(distance * dir.getXOffset(), distance * dir.getYOffset(), distance * dir.getZOffset());
            }
            default: {
                return Vec3d.ZERO;
            }
    ...
    public float getToolRenderAngle(TurtleSide side, float f) {
        return (side == TurtleSide.Left && m_animation == TurtleAnimation.SwingLeftTool) ||
               ...
               45.0f * (float) Math.sin(getAnimationFraction(f) * Math.PI) : 0.0f;
    }
            ...
            if (world.isRemote && m_animation == TurtleAnimation.MoveForward && m_animationProgress == 4) {
                ...
                if (currentHoliday == Holiday.Valentines) {
                    ...
                    if (position != null) {
                        ...
                        world.spawnParticle(EnumParticleTypes.HEART, x, y, z, world.rand.nextGaussian() * 0.02,
                                            world.rand.nextGaussian() * 0.02, world.rand.nextGaussian() * 0.02);
                    }
                }
            }

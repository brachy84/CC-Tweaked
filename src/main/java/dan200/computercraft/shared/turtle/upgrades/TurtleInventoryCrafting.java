...
public class TurtleInventoryCrafting extends InventoryCrafting {
    ...
    private IRecipe tryCrafting(int xStart, int yStart) {
        ...
        for (int x = 0; x < TileTurtle.INVENTORY_WIDTH; x++) {
            for (int y = 0; y < TileTurtle.INVENTORY_HEIGHT; y++) {
                if (x < m_xStart || x >= m_xStart + 3 || y < m_yStart || y >= m_yStart + 3) {
    ...
    public ItemStack getStackInRowAndColumn(int x, int y) {
        return x >= 0 && x < getWidth() && y >= 0 && y < getHeight() ? getStackInSlot(x + y * getWidth()) : ItemStack.EMPTY;
    }
    ...
    private int modifyIndex(int index) {
        ...
        return x >= 0 && x < TileTurtle.INVENTORY_WIDTH && y >= 0 && y < TileTurtle.INVENTORY_HEIGHT ? x + y * TileTurtle.INVENTORY_WIDTH :
               -1;
    }

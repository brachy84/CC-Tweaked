...
public abstract class ItemTurtleBase extends ItemComputerBase implements ITurtleItem {
    ...
    public void getSubItems(@Nullable CreativeTabs tabs, @Nonnull NonNullList<ItemStack> list) {
        ...
        TurtleUpgrades.getVanillaUpgrades().filter(x -> TurtleUpgrades.suitableForFamily(family, x)).map(
            x -> TurtleItemFactory.create(-1, null, -1, family, null, x, 0, null)).filter(x -> !x.isEmpty() && x.getItem() == this).forEach(
            list::add);
    }
    ...
    public String getItemStackDisplayName(@Nonnull ItemStack stack) {
        ...
        if (left != null && right != null) {
            return StringUtil.translateFormatted(baseString + ".upgraded_twice.name", StringUtil.translate(right.getUnlocalisedAdjective()),
                                                 StringUtil.translate(left.getUnlocalisedAdjective()));
        } else if (left != null) {
            return StringUtil.translateFormatted(baseString + ".upgraded.name", StringUtil.translate(left.getUnlocalisedAdjective()));
        } else if (right != null) {
            return StringUtil.translateFormatted(baseString + ".upgraded.name", StringUtil.translate(right.getUnlocalisedAdjective()));
        } else {
    ...
    public ItemStack withFamily(@Nonnull ItemStack stack, @Nonnull ComputerFamily family) {
        return TurtleItemFactory.create(getComputerID(stack), getLabel(stack), getColour(stack), family, getUpgrade(stack, TurtleSide.Left),
    ...
    public ItemStack withColour(ItemStack stack, int colour) {
        return TurtleItemFactory.create(getComputerID(stack), getLabel(stack), colour, getFamily(stack), getUpgrade(stack, TurtleSide.Left),
                                        getUpgrade(stack, TurtleSide.Right), getFuelLevel(stack), getOverlay(stack));
    }
}

...
public final class CommandComputerCraft extends CommandDelegate {
    ...
    private static ISubCommand create() {
        ...
        root.register(new SubCommandBase("dump", UserLevel.OWNER_OP) {
            ...
            public void execute(@Nonnull CommandContext context, @Nonnull List<String> arguments) throws CommandException {
                if (arguments.isEmpty()) {
                    ...
                    for (ServerComputer computer : computers) {
                        table.row(linkComputer(context, computer, computer.getID()), bool(computer.isOn()),
            ...
            public List<String> getCompletion(@Nonnull CommandContext context, @Nonnull List<String> arguments) {
                return arguments.size() == 1 ? ComputerSelector.completeComputer(arguments.get(0)) : Collections.emptyList();
            }
        });
        ...
        root.register(new SubCommandBase("shutdown", UserLevel.OWNER_OP) {
            ...
            @Override
            public List<String> getCompletion(@Nonnull CommandContext context, @Nonnull List<String> arguments) {
                return arguments.isEmpty() ? Collections.emptyList() :
        ...
        root.register(new SubCommandBase("tp", UserLevel.OP) {
            ...
            public void execute(@Nonnull CommandContext context, @Nonnull List<String> arguments) throws CommandException {
                ...
                if (sender instanceof EntityPlayerMP entity) {
        ...
        root.register(new CommandRoot("track").register(new SubCommandBase("start", UserLevel.OWNER_OP) {
            ...
            public void execute(@Nonnull CommandContext context, @Nonnull List<String> arguments) {
                ...
                context.getSender().sendMessage(list(translate("commands.computercraft.track.start.stop",
                                                               link(text(stopCommand), stopCommand,
                                                                    translate("commands.computercraft.track.stop.action")))));
            }
    ...
    private static ITextComponent linkComputer(CommandContext context, ServerComputer serverComputer, int computerId) {
        ...
        if (serverComputer == null) {
            ...
            out.appendSibling(
                link(text(Integer.toString(serverComputer.getInstanceID())), "/computercraft dump " + serverComputer.getInstanceID(),
        ...
        if (serverComputer != null && UserLevel.OP.canExecute(context) && context.fromPlayer()) {
            out.appendText(" ").appendSibling(link(text("\u261b"), "/computercraft tp " + serverComputer.getInstanceID(),
                                                   translate("commands.computercraft.tp.action"))).appendText(" ").appendSibling(
                link(text("\u20e2"), "/computercraft view " + serverComputer.getInstanceID(),
    ...
    private static ITextComponent linkPosition(CommandContext context, ServerComputer computer) {
        if (UserLevel.OP.canExecute(context)) {
            return link(position(computer.getPosition()), "/computercraft tp " + computer.getInstanceID(),
    ...
    private static void displayTimings(CommandContext context, List<ComputerTracker> timings, TrackingField field) throws CommandException {
        ...
        boolean defaultLayout = field == TrackingField.TASKS ||
            field == TrackingField.TOTAL_TIME ||
            field == TrackingField.AVERAGE_TIME ||
            field == TrackingField.MAX_TIME;


        TableBuilder table = defaultLayout ? new TableBuilder(TRACK_ID, translate("commands.computercraft.track.dump.computer"),
                             ...
                             new TableBuilder(TRACK_ID, translate("commands.computercraft.track.dump.computer"),
        ...
        for (ComputerTracker entry : timings) {
            ...
            if (defaultLayout) {
                table.row(computerComponent, text(entry.getFormatted(TrackingField.TASKS)),
                          text(entry.getFormatted(TrackingField.TOTAL_TIME)), text(entry.getFormatted(TrackingField.AVERAGE_TIME)),

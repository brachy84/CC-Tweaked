...
public final class ThreadUtils {
     ...
     * Each thread will be of the format {@code ComputerCraft-<name>-<number>}, and belong to a group called {@code ComputerCraft-<name>}
     * (which in turn will be a child group of the main {@code ComputerCraft} group.
     *
    ...
    public static ThreadFactoryBuilder builder(String name) {
        ...
        return new ThreadFactoryBuilder().setDaemon(true).setNameFormat(
            group.getName().replace("%", "%%") + "-%d").setUncaughtExceptionHandler(
            (t, e) -> ComputerCraft.log.error("Exception in thread " + t.getName(), e)).setThreadFactory(x -> new Thread(group, x));
    }

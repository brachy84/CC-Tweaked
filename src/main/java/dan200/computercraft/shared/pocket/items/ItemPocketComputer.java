...
public class ItemPocketComputer extends Item implements IComputerItem, IMedia, IColouredItem {
    ...
    public String getItemStackDisplayName(@Nonnull ItemStack stack) {
        ...
        IPocketUpgrade upgrade = getUpgrade(stack);
        if (upgrade != null) {
            return StringUtil.translateFormatted(baseString + ".upgraded.name", StringUtil.translate(upgrade.getUnlocalisedAdjective()));
        } else {
    ...
    private PocketServerComputer createServerComputer(final World world, IInventory inventory, Entity entity, @Nonnull ItemStack stack) {
        ...
        if (instanceID >= 0 && sessionID == correctSessionID && ComputerCraft.serverComputerRegistry.contains(instanceID)) {
            ...
            computer = new PocketServerComputer(world, computerID, getLabel(stack), instanceID, getFamily(stack));
    ...
    private static final IItemPropertyGetter COMPUTER_COLOURED = (stack, world, player) -> IColouredItem.getColourBasic(stack) != -1 ? 1 :
                                                                                           0;
}

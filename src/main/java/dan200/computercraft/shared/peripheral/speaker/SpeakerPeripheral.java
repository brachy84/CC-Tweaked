...
public abstract class SpeakerPeripheral implements IPeripheral {
    ...
    @Override
    public Object[] callMethod(@Nonnull IComputerAccess computerAccess, @Nonnull ILuaContext context, int methodIndex,
                               @Nonnull Object[] args) throws LuaException {
        switch (methodIndex) {
            case 0: // playSound
            {
    ...
    private synchronized boolean playSound(ILuaContext context, String name, float volume, float pitch, boolean isNote)
        throws LuaException {
        ...
        context.issueMainThreadTask(() -> {
            ...
            server.getPlayerList().sendToAllNearExcept(null, pos.x, pos.y, pos.z, adjVolume > 1.0f ? 16 * adjVolume : 16.0,
                                                       world.provider.getDimension(),
                                                       new SPacketCustomSound(name, SoundCategory.RECORDS, pos.x, pos.y, pos.z, adjVolume,
                                                                              pitch));
            return null;
        });

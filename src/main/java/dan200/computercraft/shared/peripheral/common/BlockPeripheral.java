...
public class BlockPeripheral extends BlockGeneric {
    ...
    public BlockPeripheral() {
        ...
        setDefaultState(
            blockState.getBaseState().withProperty(FACING, EnumFacing.NORTH).withProperty(VARIANT, BlockPeripheralVariant.DiskDriveEmpty));
    }
    ...
    public IBlockState getActualState(@Nonnull IBlockState state, IBlockAccess world, BlockPos pos) {
        ...
        switch (type) {
            case DiskDrive: {
                if (!(tile instanceof TileDiskDrive drive)) return state;
            ...
            case Printer: {
                if (!(tile instanceof TilePrinter printer)) return state;
            ...
            case WirelessModem: {
                if (!(tile instanceof TileWirelessModem modem)) return state;

                EnumFacing direction = modem.getDirection();
                switch (direction) {
                    case UP:
                        return state.withProperty(FACING, EnumFacing.NORTH).withProperty(VARIANT, modem.isOn() ?
                                                                                                  BlockPeripheralVariant.WirelessModemUpOn :
                                                                                                  BlockPeripheralVariant.WirelessModemUpOff);
                    case DOWN:
                        return state.withProperty(FACING, EnumFacing.NORTH).withProperty(VARIANT, modem.isOn() ?
                                                                                                  BlockPeripheralVariant.WirelessModemDownOn :
                                                                                                  BlockPeripheralVariant.WirelessModemDownOff);
                    default: {
                        return state.withProperty(FACING, direction).withProperty(VARIANT,
                                                                                  modem.isOn() ? BlockPeripheralVariant.WirelessModemOn :
                                                                                  BlockPeripheralVariant.WirelessModemOff);
                    }
                }
            }
            case Speaker: {
            ...
            case Monitor:
            case AdvancedMonitor: {
                if (!(tile instanceof TileMonitor monitor)) return state;
                ...
                if (front == EnumFacing.UP) {
                    baseVariant = type == PeripheralType.AdvancedMonitor ? BlockPeripheralVariant.AdvancedMonitorUp :
                                  BlockPeripheralVariant.MonitorUp;
                } else if (front == EnumFacing.DOWN) {
                    baseVariant = type == PeripheralType.AdvancedMonitor ? BlockPeripheralVariant.AdvancedMonitorDown :
                                  BlockPeripheralVariant.MonitorDown;
                } else {
                    baseVariant =
                        type == PeripheralType.AdvancedMonitor ? BlockPeripheralVariant.AdvancedMonitor : BlockPeripheralVariant.Monitor;
                }
                ...
                return state.withProperty(FACING, dir).withProperty(VARIANT,
    ...
    public final IBlockState getStateForPlacement(World world, BlockPos pos, EnumFacing placedSide, float hitX, float hitY, float hitZ,
        ...
        switch (getPeripheralType(damage)) {
            case DiskDrive:
            default:
                return getDefaultState().withProperty(VARIANT, BlockPeripheralVariant.DiskDriveEmpty).withProperty(FACING,
                                                                                                                   placedSide.getAxis() ==
                                                                                                                       EnumFacing.Axis.Y ?
                                                                                                                   EnumFacing.NORTH :
                                                                                                                   placedSide);
            case WirelessModem: {
                ...
                if (dir == EnumFacing.DOWN) {
                    return getDefaultState().withProperty(VARIANT, BlockPeripheralVariant.WirelessModemDownOff).withProperty(FACING,
                                                                                                                             EnumFacing.NORTH);
                } else if (dir == EnumFacing.UP) {
                    return getDefaultState().withProperty(VARIANT, BlockPeripheralVariant.WirelessModemUpOff).withProperty(FACING,
                                                                                                                           EnumFacing.NORTH);
                } else {
                    return getDefaultState().withProperty(VARIANT, BlockPeripheralVariant.WirelessModemOff).withProperty(FACING, dir);
                }
            }
            case Monitor:
    ...
    public void onBlockPlacedBy(World world, BlockPos pos, IBlockState state, EntityLivingBase player, @Nonnull ItemStack stack) {
        ...
        switch (getPeripheralType(state)) {
            ...
            case DiskDrive:
            case Printer:
                if (tile instanceof TilePeripheralBase peripheral) {
    ...
    public final boolean isOpaqueCube(IBlockState state) {
        PeripheralType type = getPeripheralType(state);
        return type == PeripheralType.DiskDrive ||
            type == PeripheralType.Printer ||
            type == PeripheralType.Monitor ||
            type == PeripheralType.AdvancedMonitor ||
    ...
    public ItemStack getPickBlock(@Nonnull IBlockState state, RayTraceResult target, @Nonnull World world, @Nonnull BlockPos pos,
                                  EntityPlayer player) {
        TileEntity tile = world.getTileEntity(pos);
        return tile instanceof ITilePeripheral ? PeripheralItemFactory.create((ITilePeripheral) tile) :

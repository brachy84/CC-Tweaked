...
public class TileMonitor extends TileGeneric implements ITilePeripheral, IPeripheralTile {
    ...
    public void onLoad() {
        super.onLoad();
        m_advanced = getBlockState().getValue(BlockPeripheral.VARIANT).getPeripheralType() == PeripheralType.AdvancedMonitor;
    ...
    protected final void readDescription(@Nonnull NBTTagCompound nbt) {
        ...
        if (oldXIndex != m_xIndex || oldYIndex != m_yIndex || oldWidth != m_width || oldHeight != m_height || oldDir != m_dir) {
    ...
    private TileMonitor getSimilarMonitorAt(BlockPos pos) {
        ...
        if (!(tile instanceof TileMonitor monitor)) return null;

        return !monitor.visiting && monitor.getDir() == getDir() && monitor.m_advanced == m_advanced && !monitor.m_destroyed ? monitor :
               null;
    }
    ...
    public void monitorTouched(float xPos, float yPos, float zPos) {
        ...
        int xCharPos = (int) Math.min(originTerminal.getWidth(),
                                      Math.max((pair.x - RENDER_BORDER - RENDER_MARGIN) / xCharWidth + 1.0, 1.0));
        int yCharPos = (int) Math.min(originTerminal.getHeight(),
                                      Math.max((pair.y - RENDER_BORDER - RENDER_MARGIN) / yCharHeight + 1.0, 1.0));

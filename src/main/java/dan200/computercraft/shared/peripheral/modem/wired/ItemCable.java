...
public class ItemCable extends ItemPeripheralBase {
    ...
    @Nonnull
    @Override
    public EnumActionResult onItemUse(@Nonnull EntityPlayer player, World world, @Nonnull BlockPos pos, @Nonnull EnumHand hand,
                                      @Nonnull EnumFacing side, float fx, float fy, float fz) {
        ...
        if (existing == ComputerCraft.Blocks.cable) {
            ...
            if (existingType == PeripheralType.WiredModem && type == PeripheralType.Cable) {
                if (!stack.isEmpty()) {
                    ...
                    world.playSound(null, pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5, soundType.getPlaceSound(),
        ...
        if (!existing.isAir(existingState, world, pos) && (type == PeripheralType.Cable || existingState.isSideSolid(world, pos, side))) {
            ...
            if (offsetExisting == ComputerCraft.Blocks.cable) {
                ...
                if (offsetExistingType == PeripheralType.Cable && type == PeripheralType.WiredModem) {
                    if (!stack.isEmpty()) {
                        IBlockState newState = offsetExistingState.withProperty(BlockCable.MODEM,
                                                                                BlockCableModemVariant.from(side.getOpposite()));
                        world.setBlockState(offset, newState, 3);
                        SoundType soundType = newState.getBlock().getSoundType(newState, world, offset, player);
                        world.playSound(null, offset.getX() + 0.5, offset.getY() + 0.5, offset.getZ() + 0.5, soundType.getPlaceSound(),
                                        SoundCategory.BLOCKS, (soundType.getVolume() + 1.0F) / 2.0F, soundType.getPitch() * 0.8F);
                        stack.shrink(1);

...
public class TileCable extends TileGeneric implements IPeripheralTile {
    ...
    private final WiredModemPeripheral m_modem = new WiredModemPeripheral(new ModemState(() -> TickScheduler.schedule(this)), m_cable) {
    ...
    public boolean onActivate(EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        if (!getWorld().isRemote) {
            ...
            if (!Objects.equal(newName, oldName)) {
                if (oldName != null) {
                    player.sendMessage(new TextComponentTranslation("chat.computercraft.wired_modem.peripheral_disconnected",
                                                                    CommandCopy.createCopyText(oldName)));
                }
                if (newName != null) {
                    player.sendMessage(new TextComponentTranslation("chat.computercraft.wired_modem.peripheral_connected",
    ...
    public <T> T getCapability(@Nonnull Capability<T> capability, @Nullable EnumFacing facing) {
        if (capability == CapabilityWiredElement.CAPABILITY) {
            return !m_destroyed && BlockCable.canConnectIn(getBlockState(), facing) ? CapabilityWiredElement.CAPABILITY.cast(m_cable) :
                   null;
        }

...
public class BlockCable extends BlockGeneric {
    ...
    static final EnumMap<EnumFacing, PropertyBool> CONNECTIONS = new EnumMap<>(
        new ImmutableMap.Builder<EnumFacing, PropertyBool>().put(EnumFacing.DOWN, DOWN).put(EnumFacing.UP, UP).put(EnumFacing.NORTH,
                                                                                                                   NORTH).put(
            EnumFacing.SOUTH, SOUTH).put(EnumFacing.WEST, WEST).put(EnumFacing.EAST, EAST).build());
    ...
    public BlockCable() {
        ...
        setDefaultState(
            blockState.getBaseState().withProperty(MODEM, BlockCableModemVariant.None).withProperty(CABLE, false).withProperty(NORTH,
                                                                                                                               false).withProperty(
                SOUTH, false).withProperty(EAST, false).withProperty(WEST, false).withProperty(UP, false).withProperty(DOWN, false));
    }

    @Nonnull
    @Override
    protected BlockStateContainer createBlockState() {
        return new BlockStateContainer(this, MODEM, CABLE, NORTH, SOUTH, EAST, WEST, UP, DOWN);
    }
    ...
    @Override
    @Deprecated
    public final IBlockState getStateForPlacement(World world, BlockPos pos, EnumFacing placedSide, float hitX, float hitY, float hitZ,
                                                  int damage, EntityLivingBase placer) {
        switch (ComputerCraft.Items.cable.getPeripheralType(damage)) {
            case Cable:
                return getDefaultState().withProperty(CABLE, true).withProperty(MODEM, BlockCableModemVariant.None);
            default:
            case WiredModem:
                return getDefaultState().withProperty(CABLE, false).withProperty(MODEM,
                                                                                 BlockCableModemVariant.from(placedSide.getOpposite()));
            case WiredModemWithCable:
                return getDefaultState().withProperty(CABLE, true).withProperty(MODEM,
                                                                                BlockCableModemVariant.from(placedSide.getOpposite()));
        }
    }

    public static boolean canConnectIn(IBlockState state, EnumFacing direction) {
        return state.getValue(CABLE) && state.getValue(MODEM).getFacing() != direction;
    }
    ...
    public IBlockState getActualState(@Nonnull IBlockState state, IBlockAccess world, BlockPos pos) {
        state = state.withProperty(NORTH, doesConnectVisually(state, world, pos, EnumFacing.NORTH)).withProperty(SOUTH,
                                                                                                                 doesConnectVisually(state,
                                                                                                                                     world,
                                                                                                                                     pos,
                                                                                                                                     EnumFacing.SOUTH)).withProperty(
            EAST, doesConnectVisually(state, world, pos, EnumFacing.EAST)).withProperty(WEST, doesConnectVisually(state, world, pos,
                                                                                                                  EnumFacing.WEST)).withProperty(
            UP, doesConnectVisually(state, world, pos, EnumFacing.UP)).withProperty(DOWN, doesConnectVisually(state, world, pos,
                                                                                                              EnumFacing.DOWN));
    ...
    @Override
    @Deprecated
    public void addCollisionBoxToList(IBlockState state, @Nonnull World world, @Nonnull BlockPos pos, @Nonnull AxisAlignedBB bigBox,
                                      @Nonnull List<AxisAlignedBB> list, Entity entity, boolean isActualState) {
    ...
    @Override
    @Deprecated
    public RayTraceResult collisionRayTrace(IBlockState state, @Nonnull World world, @Nonnull BlockPos pos, @Nonnull Vec3d start,
                                            @Nonnull Vec3d end) {
    ...
    @Override
    public boolean removedByPlayer(@Nonnull IBlockState state, World world, @Nonnull BlockPos pos, @Nonnull EntityPlayer player,
                                   boolean willHarvest) {
    ...
    @Nonnull
    @Override
    public ItemStack getPickBlock(@Nonnull IBlockState state, RayTraceResult hit, @Nonnull World world, @Nonnull BlockPos pos,
                                  EntityPlayer player) {
        PeripheralType type = getPeripheralType(state);
        if (type == PeripheralType.WiredModemWithCable) {
            type = hit == null ||
                       WorldUtil.isVecInsideInclusive(CableBounds.getModemBounds(state),
                                                      hit.hitVec.subtract(pos.getX(), pos.getY(), pos.getZ())) ? PeripheralType.WiredModem :
                   PeripheralType.Cable;
        }

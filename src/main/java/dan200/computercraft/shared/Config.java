...
public final class Config {
    ...
    public static void load(File configFile) {
        ...
        { // General computers
            ...
            commandRequireCreative.setComment(
            ...
            setOrder(CATEGORY_GENERAL, computerSpaceLimit, floppySpaceLimit, maximumFilesOpen, disableLua51Features,
                     defaultComputerSettings, debugEnabled, logComputerErrors, commandRequireCreative);
        }

        { // Execution
            renameProperty(CATEGORY_GENERAL, "computer_threads", CATEGORY_EXECUTION, "computer_threads");

            config.getCategory(CATEGORY_EXECUTION).setComment(
            ...
            computerThreads.setMinValue(1).setRequiresMcRestart(true).setComment(
                "Set the number of threads computers can run on. A higher number means more computers can " +
                    "run at once, but may induce lag.\n" +
                    "Please note that some mods may not work with a thread count higher than 1. Use with caution.");

            maxMainGlobalTime = config.get(CATEGORY_EXECUTION, "max_main_global_time",
                                           (int) TimeUnit.NANOSECONDS.toMillis(ComputerCraft.maxMainGlobalTime));
            maxMainGlobalTime.setMinValue(1).setComment(
            ...
            maxMainComputerTime = config.get(CATEGORY_EXECUTION, "max_main_computer_time",
                                             (int) TimeUnit.NANOSECONDS.toMillis(ComputerCraft.maxMainComputerTime));
            maxMainComputerTime.setMinValue(1).setComment(
                "The ideal maximum time a computer can execute for in a tick, in milliseconds.\n" +
                    "Note, we will quite possibly go over this limit, as there's no way to tell how long a will take - this aims " +
                    "to be the upper bound of the average time.");

            setOrder(CATEGORY_EXECUTION, computerThreads, maxMainGlobalTime, maxMainComputerTime);
        }
        ...
        { // HTTP
            ...
            config.getCategory(CATEGORY_HTTP).setComment("Controls the HTTP API");
            ...
            setOrder(CATEGORY_HTTP, httpEnable, httpWebsocketEnable, httpAllowedDomains, httpBlockedDomains, httpTimeout, httpMaxRequests,
                     httpMaxDownload, httpMaxUpload, httpMaxWebsockets, httpMaxWebsocketMessage);
        }
        ...
        { // Peripherals
            ...
            renameProperty(CATEGORY_GENERAL, "modem_highAltitudeRangeDuringStorm", CATEGORY_PERIPHERAL,
                           "modem_high_altitude_range_during_storm");
            renameProperty(CATEGORY_GENERAL, "maxNotesPerTick", CATEGORY_PERIPHERAL, "max_notes_per_tick");

            config.getCategory(CATEGORY_PERIPHERAL).setComment("Various options relating to peripherals.");
            ...
            modemHighAltitudeRangeDuringStorm = config.get(CATEGORY_PERIPHERAL, "modem_high_altitude_range_during_storm",
            ...
            setOrder(CATEGORY_PERIPHERAL, commandBlockEnabled, modemRange, modemHighAltitudeRange, modemRangeDuringStorm,
                     modemHighAltitudeRangeDuringStorm, maxNotesPerTick, monitorRenderer, monitorBandwidth);
        }
        ...
        { // Turtles
            ...
            config.getCategory(CATEGORY_TURTLE).setComment("Various options relating to turtles.");
            ...
            setOrder(CATEGORY_TURTLE, turtlesNeedFuel, turtleFuelLimit, advancedTurtleFuelLimit, turtlesObeyBlockProtection, turtlesCanPush,
                     turtleDisabledActions);
        }

        for (String child : config.getCategoryNames()) {
            setupLanguage(config.getCategory(child),
    ...
    private static void reloadCategory(ConfigCategory oldCat, ConfigCategory newCat) {
        ...
        for (Map.Entry<String, Property> child : newCat.getValues().entrySet()) {
            ...
            if (oldProperty.getType() != newProperty.getType() || oldProperty.isList() != newProperty.isList()) {
                ComputerCraft.log.warn("Cannot sync config property {} (type changed from {} to {})", child.getKey(), getType(oldProperty),
                                       getType(newProperty));
                continue;
            }

...
public class WidgetTerminal extends Widget {
    ...
    public WidgetTerminal(int x, int y, int termWidth, int termHeight, IComputerContainer computer, int leftMargin, int rightMargin,
                          int topMargin, int bottomMargin) {
        super(x, y, leftMargin + rightMargin + termWidth * FixedWidthFontRenderer.FONT_WIDTH,
    ...
    public void mouseClicked(int mouseX, int mouseY, int button) {
        if (mouseX >= getXPosition() &&
            mouseX < getXPosition() + getWidth() &&
            mouseY >= getYPosition() &&
            mouseY < getYPosition() + getHeight()) {
            if (!m_focus && button == 0) {
                m_focus = true;
            }
    ...
    public void draw(Minecraft mc, int xOrigin, int yOrigin, int mouseX, int mouseY) {
        ...
        synchronized (m_computer) {
            ...
            if (terminal != null) {
                FixedWidthFontRenderer.drawTerminal(startX + topMargin, startY + bottomMargin, terminal, !computer.isColour(), topMargin,

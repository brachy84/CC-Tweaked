...
public class TileEntityMonitorRenderer extends TileEntitySpecialRenderer<TileMonitor> {
    ...
    private static void renderMonitorAt(TileMonitor monitor, double posX, double posY, double posZ, float f, int i) {
        ...
        GlStateManager.translate(-0.5 + TileMonitor.RENDER_BORDER + RENDER_MARGIN,
                                 origin.getHeight() - 0.5 - (TileMonitor.RENDER_BORDER + RENDER_MARGIN), 0.5);
        ...
        if (terminal != null) {
            ...
            FixedWidthFontRenderer.drawEmptyTerminal(-MARGIN, MARGIN, (float) (xSize + 2 * MARGIN), (float) -(ySize + MARGIN * 2));
        }
        ...
        FixedWidthFontRenderer.drawBlocker((float) -TileMonitor.RENDER_MARGIN, (float) TileMonitor.RENDER_MARGIN,
            ...
            case TBO: {
                ...
                if (redraw) {
                    ...
                    for (int y = 0; y < height; y++) {
                        TextBuffer text = terminal.getLine(y), textColour = terminal.getTextColourLine(
                            y), background = terminal.getBackgroundColourLine(y);
    ...
    private static void renderTerminalTo(ClientMonitor monitor, BufferBuilder buffer, float xMargin, float yMargin) {
        FixedWidthFontRenderer.begin(buffer);
        FixedWidthFontRenderer.drawTerminalWithoutCursor(buffer, 0, 0, monitor.getTerminal(), !monitor.isColour(), yMargin, yMargin,
                                                         xMargin, xMargin);
    }

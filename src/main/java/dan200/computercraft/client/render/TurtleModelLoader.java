...
public final class TurtleModelLoader implements ICustomModelLoader {
    ...
    private static final class TurtleModel implements IModel {
        ...
        public IBakedModel bake(@Nonnull IModelState state, @Nonnull VertexFormat format,
                                @Nonnull Function<ResourceLocation, TextureAtlasSprite> function) {
            return new TurtleSmartItemModel(family.bake(state, format, function), colour.bake(state, format, function));
        }

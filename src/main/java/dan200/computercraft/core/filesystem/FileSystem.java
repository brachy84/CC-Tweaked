...
public class FileSystem {
     ...
     * This is a pretty arbitrary value, though hopefully it is large enough that it'll never be normally hit. This exists to prevent it
     * overflowing if it ever gets into an infinite loop.
     */
    ...
    private synchronized void copyRecursive(String sourcePath, MountWrapper sourceMount, String destinationPath,
                                            MountWrapper destinationMount, int depth) throws FileSystemException {
        ...
        if (sourceMount.isDirectory(sourcePath)) {
            ...
            for (String child : sourceChildren) {
                copyRecursive(combine(sourcePath, child), sourceMount, combine(destinationPath, child), destinationMount, depth + 1);
            }
        } else {
            // Copy a file:
            try (ReadableByteChannel source = sourceMount.openForRead(
                sourcePath); WritableByteChannel destination = destinationMount.openForWrite(destinationPath)) {
    ...
    private synchronized <T extends Closeable> FileSystemWrapper<T> openFile(@Nonnull Channel channel, @Nonnull T file)
        throws FileSystemException {
        synchronized (m_openFiles) {
            if (ComputerCraft.maximumFilesOpen > 0 && m_openFiles.size() >= ComputerCraft.maximumFilesOpen) {
    ...
    public synchronized <T extends Closeable> FileSystemWrapper<T> openForRead(String path, Function<ReadableByteChannel, T> open)
        throws FileSystemException {
        cleanup();
    ...
    public synchronized <T extends Closeable> FileSystemWrapper<T> openForWrite(String path, boolean append,
                                                                                Function<WritableByteChannel, T> open)
        throws FileSystemException {
        cleanup();
    ...
    private static String sanitizePath(String path, boolean allowWildcards) {
        ...
        final char[] specialChars = new char[]{'"', ':', '<', '>', '?', '|', // Sorted by ascii value (important)
        };

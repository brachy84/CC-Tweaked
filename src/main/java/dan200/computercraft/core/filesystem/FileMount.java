...
public class FileMount implements IWritableMount {
    ...
    private static final Set<OpenOption> WRITE_OPTIONS = Sets.newHashSet(StandardOpenOption.WRITE, StandardOpenOption.CREATE,
                                                                         StandardOpenOption.TRUNCATE_EXISTING);
    private static final Set<OpenOption> APPEND_OPTIONS = Sets.newHashSet(StandardOpenOption.WRITE, StandardOpenOption.CREATE,
    ...
    public void makeDirectory(@Nonnull String path) throws IOException {
        ...
        if (getRemainingSpace() < (long) dirsToCreate * MINIMUM_FILE_SIZE) {
            throw new FileOperationException(path, "Out of space");
        }

        if (file.mkdirs()) {
            m_usedSpace += (long) dirsToCreate * MINIMUM_FILE_SIZE;
        } else {
    ...
    public WritableByteChannel openChannelForAppend(@Nonnull String path) throws IOException {
        ...
        return new WritableCountingChannel(Files.newByteChannel(file.toPath(), APPEND_OPTIONS),

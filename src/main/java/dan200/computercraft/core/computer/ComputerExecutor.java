 ...
 * When the computer is instructed to turn on or off, or handle an event, we queue a task and register this to be executed on the
 * {@link ComputerThread}. Note, as we may be starting many events in a single tick, the external cannot lock on anything which may be held
 * for a long time.
 * The executor is effectively composed of two separate queues. Firstly, we have a "single element" queue {@link #command} which determines
 * which state the computer should transition too. This is set by {@link #queueStart()} and {@link #queueStop(boolean, boolean)}.
 * When a computer is on, we simply push any events onto to the {@link #eventQueue}.
 * Both queues are run from the {@link #work()} method, which tries to execute a command if one exists, or resumes the machine with an event
 * otherwise.
 * One final responsibility for the executor is calling {@link ILuaAPI#update()} every tick, via the {@link #tick()} method. This should
 * only be called when the computer is actually on ({@link #isOn}).
 */
final class ComputerExecutor {
    ...
    /**
     * Whether the computer is currently on. This is set to false when a shutdown starts, or when turning on completes (but just before the
     * Lua machine is started).
     *
     * @see #isOnLock
     */
    ...
    /**
     * A lock used for any changes to {@link #eventQueue}, {@link #command} or {@link #onComputerQueue}. This will be used on the main
     ...
     * One sets the command with {@link #queueStart()} and {@link #queueStop(boolean, boolean)}. Neither of these will queue a new event if
     * there is an existing one in the queue.
     * Note, if command is not {@code null}, then some command is scheduled to be executed. Otherwise it is not currently in the queue (or
    ...
    /**
     * The thread the executor is running on. This is non-null when performing work. We use this to ensure we're only doing one bit of work
     * at one time.
     *
     * @see ComputerThread
     */
    ...
    IWritableMount getRootMount() {
        if (rootMount == null) {
            rootMount = computer.getComputerEnvironment().createSaveDirMount("computer/" + computer.assignID(),

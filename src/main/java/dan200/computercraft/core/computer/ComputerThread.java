 ...
 * This is split into two components: the {@link TaskRunner}s, which pull an executor from the queue and execute it, and a single
 * {@link Monitor} which observes all runners and kills them if they have not been terminated by {@link TimeoutState#isSoftAborted()}.
 * Computers are executed using a priority system, with those who have spent less time executing having a higher priority than those hogging
 * the thread. This, combined with {@link TimeoutState#isPaused()} means we can reduce the risk of badly behaved computers stalling
 * execution for everyone else.
 * This is done using an implementation of Linux's Completely Fair Scheduler. When a computer executes, we compute what share of execution
 * time it has used (time executed/number of tasks). We then pick the computer who has the least "virtual execution time" (aka
 * {@link ComputerExecutor#virtualRuntime}).
 * When adding a computer to the queue, we make sure its "virtual runtime" is at least as big as the smallest runtime. This means that
 * adding computers which have slept a lot do not then have massive priority over everyone else. See {@link #queue(ComputerExecutor)} for
 * how this is implemented.
 * In reality, it's unlikely that more than a few computers are waiting to execute at once, so this will not have much effect unless you
 * have a computer hogging execution time. However, it is pretty effective in those situations.
 *
...
public final class ComputerThread {
     ...
     * An average tick takes 50ms, and so we ideally need to have handled a couple of events within that window in order to have a perceived
     * low latency.
     */
     ...
     * From statistics gathered on SwitchCraft, almost all machines will execute under 15ms, 75% under 1.5ms, with the mean being about 3ms.
     * Most computers shouldn't be too impacted with having such a short period to execute in.
     */
     ...
     * You must be holding {@link ComputerExecutor}'s {@code queueLock} when calling this method - it should only be called from
     * {@code enqueue}.
     *
    ...
    /**
     * Update the {@link ComputerExecutor#virtualRuntime}s of all running tasks, and then update the {@link #minimumVirtualRuntime} based on
     * the current tasks.
    ...
    /**
     * Check if we have more work queued than we have capacity for. Effectively a more fine-grained version of {@link #hasPendingWork()}.
     *
     ...
     * {@link ComputerExecutor#afterWork()} functions. Everything else is either handled by the executor, timeout state or monitor.
     */
    private static final class TaskRunner implements Runnable {
        ...
        private void reportTimeout(ComputerExecutor executor, long time) {
            ...
            StringBuilder builder = new StringBuilder().append("Terminating computer #").append(executor.getComputer().getID()).append(
                " due to timeout (running for ").append(time * 1e-9).append(
                " seconds). This is NOT a bug, but may mean a computer is misbehaving. ").append(owner.getName()).append(
                " is currently ").append(owner.getState());

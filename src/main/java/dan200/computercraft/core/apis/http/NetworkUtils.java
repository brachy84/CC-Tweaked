...
public final class NetworkUtils {
    ...
    public static final ExecutorService EXECUTOR = new ThreadPoolExecutor(4, Integer.MAX_VALUE, 60L, TimeUnit.SECONDS,
                                                                          new SynchronousQueue<>(),
                                                                          ThreadUtils.builder("Network").setPriority(Thread.MIN_PRIORITY +
                                                                                                                         (Thread.NORM_PRIORITY -
                                                                                                                              Thread.MIN_PRIORITY) /
                                                                                                                             2).build());

    public static final EventLoopGroup LOOP_GROUP = new NioEventLoopGroup(4, ThreadUtils.builder("Netty").setPriority(
        Thread.MIN_PRIORITY + (Thread.NORM_PRIORITY - Thread.MIN_PRIORITY) / 2).build());
    ...
    public static SslContext getSslContext() throws HTTPRequestException {
        ...
        synchronized (sslLock) {
            if (sslContext != null || triedSslContext) return sslContext;
            try {
                return sslContext = SslContextBuilder.forClient().trustManager(getTrustManager()).build();

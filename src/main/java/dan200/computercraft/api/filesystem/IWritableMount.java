...
/**
 * Represents a part of a virtual filesystem that can be mounted onto a computer using {@link IComputerAccess#mount(String, IMount)} or
 * {@link IComputerAccess#mountWritable(String, IWritableMount)}, that can also be written to.
 * Ready made implementations of this interface can be created using {@link ComputerCraftAPI#createSaveDirMount(World, String, long)}, or
 * you're free to implement it yourselves!
 *
...
public interface IWritableMount extends IMount {
     ...
     *
     * @param path A file path in normalised format, relative to the mount location. ie: "programs/myprogram".
     * @return A stream for writing to. If the channel implements {@link java.nio.channels.SeekableByteChannel}, one will be able to seek to
     * arbitrary positions when using binary mode.
     * @throws IOException If the file could not be opened for writing.
     */
    @Nonnull
    ...
    /**
     * Get the amount of free space on the mount, in bytes. You should decrease this value as the user writes to the mount, and write
     * operations should fail once it reaches zero.
     *
    ...
    /**
     * Get the capacity of this mount. This should be equal to the size of all files/directories on this mount, minus the
     * {@link #getRemainingSpace()}.
     *

...
public interface ITurtleAccess {
     ...
     * This will preserve the turtle's internal state, such as it's inventory, computer and upgrades. It should be used before playing a
     * movement animation using {@link #playAnimation(TurtleAnimation)}.
     *
     * @param world The new world to move it to
     * @param pos   The new position to move it to.
     * @return Whether the movement was successful. It may fail if the block was not loaded or the block placement was cancelled. Note this
     * will not check {@link dan200.computercraft.api.permissions.ITurtlePermissionProvider#isBlockEnterable(World, BlockPos)}.
    ...
    /**
     * Returns a vector containing the floating point co-ordinates at which the turtle is rendered. This will shift when the turtle is
     * moving.
     *
    ...
    /**
     * Set the direction the turtle is facing. Note that this will not play a rotation animation, you will also need to call
     * {@link #playAnimation(TurtleAnimation)} to do so.
     *
     ...
     *
     * @param colour The colour this turtle should be changed to. This should be a RGB colour between {@code 0x000000} and {@code 0xFFFFFF}
     *               or -1 to reset to the default colour.
     * @see #getColour()
     */
     ...
     * @return Whether the turtle was able to consume the amount of fuel specified. Will return false if you supply a number greater than
     * the current fuel level of the turtle. No fuel will be consumed if {@code false} is returned.
    ...
    /**
     * Adds a custom command to the turtles command queue. Unlike peripheral methods, these custom commands will be executed on the main
     * thread, so are guaranteed to be able to access Minecraft objects safely, and will be queued up with the turtles standard movement and
     * tool commands. An issued command will return an unique integer, which will be supplied as a parameter to a "turtle_response" event
     * issued to the turtle after the command has completed. Look at the lua source code for "rom/apis/turtle" for how to build a lua
     ...
     * @return The objects the command returned when executed. you should probably return these to the player unchanged if called from a
     * peripheral method.
     ...
     *                                       InterruptedException will be thrown. This exception must not be caught or intercepted, or the
     *                                       computer will leak memory and end up in a broken state.
     * @see ITurtleCommand
     ...
     * This will be persisted across turtle restarts and chunk loads, as well as being synced to the client. You must call
     * {@link #updateUpgradeNBTData(TurtleSide)} after modifying it.
     *

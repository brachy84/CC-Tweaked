...
/**
 * The interface passed to peripherals by computers or turtles, providing methods that they can call. This should not be implemented by your
 * classes. Do not interact with computers except via this interface.
 */
public interface IComputerAccess {
     ...
     * @return The location on the computer's file system where you the mount mounted, or {@code null} if there was already a file in the
     * desired location. Store this value if you wish to unmount the mount later.
     ...
     * @param desiredLocation The location on the computer's file system where you would like the mount to be mounted.
     * @param mount           The mount object to mount on the computer.
     * @return The location on the computer's file system where you the mount mounted, or null if there was already a file in the desired
    ...
    /**
     * Unmounts a directory previously mounted onto the computers file system by {@link #mount(String, IMount)} or
     * {@link #mountWritable(String, IWritableMount)}.
     ...
     * All directories mounted by a mount or mountWritable are automatically unmounted when the peripheral is attached if they have not been
     * explicitly unmounted.
     * Note that you cannot unmount another peripheral's mounts.
     *
     * @param location The desired location in the computers file system of the directory to unmount. This must be the location of a
     *                 directory previously mounted by {@link #mount(String, IMount)} or {@link #mountWritable(String, IWritableMount)}, as
     ...
     * This is the same number obtained by calling {@code os.getComputerID()} or running the "id" program from lua, and is guaranteed
     * unique. This number will be positive.
     *
    ...
    /**
     * Causes an event to be raised on this computer, which the computer can respond to by calling {@code os.pullEvent()}. This can be used
     * to notify the computer when things happen in the world or to this peripheral.
     *
     * @param event     A string identifying the type of event that has occurred, this will be returned as the first value from
     *                  {@code os.pullEvent()}. It is recommended that you you choose a name that is unique, and recognisable as originating
     *                  from your peripheral. eg: If your peripheral type is "button", a suitable event would be "button_pressed".
     * @param arguments In addition to a name, you may pass an array of extra arguments to the event, that will be supplied as extra return
     *                  values to os.pullEvent(). Objects in the array will be converted to lua data types in the same fashion as the return
     *                  values of IPeripheral.callMethod().
     ...
     * "left","right","front","back",etc, but for peripherals attached remotely it will be different. It is good practice to supply this
     * string when raising events to the computer, so that the computer knows from which peripheral the event came.
     *
     ...
     * This should be used to ensure your peripheral integrates with ComputerCraft's monitoring and limiting of how much server time each
     * computer consumes. You should not need to use this if you use {@link ILuaContext#issueMainThreadTask(ILuaTask)} - this is intended
     * for mods with their own system for running work on the main thread.

...
/**
 * Monitors "work" associated with a computer, keeping track of how much a computer has done, and ensuring every computer receives a fair
 * share of any processing time.
 * This is primarily intended for work done by peripherals on the main thread (such as on a tile entity's tick), but could be used for other
 * purposes (such as complex computations done on another thread).
 * Before running a task, one should call {@link #canWork()} to determine if the computer is currently allowed to execute work. If that
 * returns true, you should execute the task and use {@link #trackWork(long, TimeUnit)} to inform the monitor how long that task took.
...
public interface IWorkMonitor {
     ...
     * This is effectively a more restrictive form of {@link #canWork()}. One should use that in order to determine if you may do an initial
     * piece of work, and shouldWork to determine if any additional task may be performed.
     *
